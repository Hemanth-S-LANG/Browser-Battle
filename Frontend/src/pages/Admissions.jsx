        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {steps.map(({ step, title, desc, icon: Icon }, i) => (
            <AnimatedSection key={i} delay={i * 0.1}>
              <div className="relative rounded-2xl p-6 shadow-sm border border-blue-500/10 hover:shadow-lg transition-shadow" style={{ background: "var(--card)", border: "1px solid var(--border)", backdropFilter: "blur(12px)" }}>
      ...
      <section className="section-padding max-w-7xl mx-auto">
        <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="rounded-2xl p-8 shadow-sm border border-blue-500/10" style={{ background: "var(--card)", border: "1px solid var(--border)", backdropFilter: "blur(12px)" }}>
              ...
              <div className="space-y-3">
                {['Application Form 2024-25', 'Prospectus 2024-25', 'Fee Structure', 'Scholarship Form'].map((form, i) => (
                  <button key={i} className="w-full flex items-center justify-between p-3 bg-[rgba(15,23,42,0.6)] rounded-xl hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors group">
                    <span className="text-sm t-h-sec flex items-center gap-2">
                      <FileText size={14} className="text-blue-500" /> {form}
                    </span>
                    <Download size={14} className="t-h-mut group-hover:text-blue-600 transition-colors" />
                  </button>
                ))}
              </div>
            </div>
          ...
          <AnimatedSection delay={0.1}>
            <div className="bg-gradient-to-br from-blue-600 to-cyan-500 rounded-2xl p-8 t-h">
              <h3 className="text-xl font-bold mb-3">Ready to Apply?</h3>
              <p className="text-blue-100 mb-6 text-sm">Applications for 2024-25 are now open. Secure your seat at RNSIT today.</p>
              <div className="space-y-3">
                <a href="#" className="flex items-center justify-center gap-2 w-full py-3 bg-white text-blue-700 font-semibold rounded-xl hover:shadow-lg transition-all">
